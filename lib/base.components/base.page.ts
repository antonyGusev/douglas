...
import { isObject, isPrimitive, isString, waitForCondition, isEqual, logger, logMessage, stringifyValue } from '../helpers';
...
export class BasePage extends AbstractComponent {
  ...
  async waitForLoaded(state?: StateType, options?: OptionsType): Promise<void> {
    ...
    await this._page!.waitForLoadState(state, options);
  }
}
